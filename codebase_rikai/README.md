...
####追加Q&A
「GitHub Actions と手元 CI で使う Claude Code の headless モードのフラグを 必ず手元で先に確認 してください。」といたちえいるの何故？　headless モード？　フラグ？　どうして手元で先に確認する必要があるの？
あと、俺はこのツールをローカルでメインで使うけど、今のところ、git hub action上では使うつもりないんだけど、大丈夫？

良い質問。両方ちゃんと答える。

## なぜ「フラグを先に確認」と書いたのか

### Claude Code の「headless モード」とは

普段あなたが使っているのは「インタラクティブモード」。ターミナルで `claude` と打つと TUI が立ち上がって、対話しながら操作するやつ。

これに対して **headless モード** は「人間の対話なしで、プログラムから1回限り Claude Code を起動して結果を取る」モード。`claude -p "プロンプトの内容"` のように引数で指示を渡すと、Claude Code が起動して指示を実行して結果を返して終了する、というワンショット実行ができる。

GitHub Actions のような CI 環境では人間が画面を見て操作することが不可能なので、headless モードで起動するしかない。だから workflow yml の中に `claude -p "..."` のような行が入っている。

### なぜ「フラグ」が問題になるのか

Claude Code は活発に開発されていて、CLI の引数（フラグ）が**バージョン間で変わる**ことがある。具体的に起こりうる差異:

- 古い版: `claude --headless "プロンプト"`
- 別の版: `claude -p "プロンプト"`
- 別の版: `claude --print "プロンプト"`
- さらに新しい版だとプロンプトを stdin から渡す方式になっている可能性もある

私が workflow yml を書いた時点では「`claude -p "..."` が動くと仮定」して書いている。でも、あなたがこの workflow を動かす時点でフラグ仕様が変わっていたら、CI が初回起動でいきなり「unknown flag」エラーで落ちる。

これは**よくある事故**で、特に GitHub Actions だと「動かない → ログ見る → CLI バージョン確認 → フラグ修正 → push → 待つ → またダメ」のサイクルが30分単位で消える。**手元で `claude --help` を1分打って先に確認しておけば回避できる**ので、README に書いた、というのが理由。

要するに「初回でハマる地雷の予告」です。

## ローカルメイン利用なら GitHub Actions は不要

**完全に不要です。** GitHub Actions 周りは無視してOK。

GitHub Actions は「2週ごとに自動でリフレッシュする」ための仕組みで、Meta 論文の数週間ごとにファイルパスを検証し、カバレッジギャップを検出し、批評を再実行し、stale参照を自動修復するを再現したかっただけ。ローカルメイン運用なら、自分が必要なときに手で `/tribal-refresh` を叩けば同じことができる。

### あなたの運用に合わせた使い方

ローカルメインなら、こうしておけば全部完結する:

**削除して良いもの:**
- `.github/workflows/tribal-refresh.yml` — 不要
- `.claude/scripts/check_quality_regression.py` — main ブランチ baseline 比較が前提なので、ローカルでは出番が少ない（ただし将来チームで使う可能性があるなら残しておいても害はない）

**残すべきもの:**
- 他の全ファイル（agents / skills / commands / settings.json / 他のスクリプト）

**フラグ確認も不要:**
ローカルでは `/tribal-init` のように Claude Code 内で slash command を叩くだけ。CLI フラグは関係ない（Claude Code を CLI から `claude -p` で起動する場面がないから）。インタラクティブセッションの中で全部完結する。

### あなたの実運用フロー

```
1. 新しい現場/プロジェクトに入る
   → そのプロジェクトのルートに .claude/ と CLAUDE.md を配置
...
3. 普段の開発
   → 普通にタスクを Claude に投げる
   → UserPromptSubmit hook が router を呼ぶ
   → router が context を 3-5 枚ロード
   → そのまま開発を進める
...
5. たまに健康診断
   /tribal-validate
   → broken path や score 低下を確認
```

GitHub Actions 周りの記述は「将来チーム展開する時のために残してある」くらいに思っておけば十分。今すぐ消したいなら `.github/workflows/tribal-refresh.yml` だけ消せば、他には何の影響もない。

### 補足: ローカル運用で気をつける点

GitHub Actions がやってくれていた「2週ごとの自動リフレッシュ」がない分、**stale 検出が遅れる**リスクは自分で管理することになる。対策はシンプルで、月1回くらい手動で `/tribal-validate` を流して健康状態を確認する習慣をつければ十分。リマインダーアプリで「毎月1日: tribal-validate」と通知出すくらいで運用は回る。
